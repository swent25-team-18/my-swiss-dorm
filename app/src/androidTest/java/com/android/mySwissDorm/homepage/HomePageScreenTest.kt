...
class HomePageScreenTest {
    ...
    fun testCityCards() {
        ...
        composeTestRule.onNodeWithText("Lausanne is a city located on Lake Geneva, known for its universities and the Olympic Museum.").assertIsDisplayed()

        // Check that the "Geneva" card is displayed
        composeTestRule.onNodeWithText("Geneva").assertIsDisplayed()
        composeTestRule.onNodeWithText("Geneva is a global city, hosting numerous international organizations.").assertIsDisplayed()

        // Check that the "Zürich" card is displayed
        composeTestRule.onNodeWithText("Zürich").assertIsDisplayed()
        composeTestRule.onNodeWithText("Zurich is the largest city in Switzerland and a major financial hub.").assertIsDisplayed()

        // Check that the "Fribourg" card is displayed
        composeTestRule.onNodeWithText("Fribourg").assertIsDisplayed()
        composeTestRule.onNodeWithText("Fribourg is a bilingual city famous for its medieval architecture.").assertIsDisplayed()
    }
