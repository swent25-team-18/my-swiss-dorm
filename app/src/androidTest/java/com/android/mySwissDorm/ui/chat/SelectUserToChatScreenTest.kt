...
class SelectUserToChatScreenTest : FirestoreTest() {
  ...
  @Test
  fun selectUserToChatScreen_excludesBlockedUsers() = runTest {
    switchToUser(FakeUser.FakeUser1)
    val currentUserId = FirebaseAuth.getInstance().currentUser?.uid ?: return@runTest
    ...
    // Switch back to current user and block the other user
    switchToUser(FakeUser.FakeUser1)
    delay(200)

    // Block the other user
    ProfileRepositoryProvider.repository.addBlockedUser(currentUserId, otherUserId)
    delay(500)
    ...
    // Should NOT display blocked user
    compose.onNodeWithText("Alice Queen").assertDoesNotExist()
