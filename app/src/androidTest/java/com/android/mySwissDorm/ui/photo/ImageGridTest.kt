...
class ImageGridTest {
  ...
  fun imageGridDisplaysAllImages() {
    composeTestRule.setContent {
      ImageGrid(imageUris = testUris, isEditingMode = false, onRemove = {})
    }
    testUris.forEach { uri ->
      composeTestRule
          .onNodeWithTag(C.ImageGridTags.imageTag(uri), useUnmergedTree = true)
          .assertIsDisplayed()
          .performScrollTo()
    }
  }
  ...
  fun imageGridDisplaysDeleteButton() {
    composeTestRule.setContent {
      ImageGrid(imageUris = testUris, isEditingMode = true, onRemove = {})
    }
    testUris.forEach { uri ->
      composeTestRule
          .onNodeWithTag(C.ImageGridTags.imageTag(uri), useUnmergedTree = true)
          ...
          .performScrollTo()
      composeTestRule
          .onNodeWithTag(C.ImageGridTags.deleteButtonTag(uri), useUnmergedTree = true)
          .assertIsDisplayed()
    }
  }
  ...
  fun imageGridDisplaysNothingOnEmptyList() {
    composeTestRule.setContent {
      ImageGrid(imageUris = setOf(), isEditingMode = true, onRemove = {})
    }

    composeTestRule
        .onNodeWithContentDescription(
            C.ImageGridTags.ICON_DELETE_CONTENT_DESC, useUnmergedTree = true)
        .assertDoesNotExist()
  }
  ...
  fun imageGridOnRemoveWorks() {
    ...
    composeTestRule.setContent {
      ImageGrid(imageUris = testUris, isEditingMode = true, onRemove = { removedUris.add(it) })
    }
