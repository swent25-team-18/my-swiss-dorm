...
class SettingsViewModelTest : FirestoreTest() {
  ...
  override fun createRepositories() { /* none needed here */ }
  ...
  @Test
  fun refresh_readsDisplayNameFromProfile_whenDocumentExists() = runTest(mainRule.scheduler) {
    // Sign in as FakeUser1
    switchToUser(FakeUser.FakeUser1)
    val uid = FirebaseEmulator.auth.currentUser!!.uid

    // Use the repository to seed a correctly shaped profile document
    val repo = com.android.mySwissDorm.model.profile.ProfileRepositoryFirestore(FirebaseEmulator.firestore)
    val profile = com.android.mySwissDorm.model.profile.Profile(
      userInfo = com.android.mySwissDorm.model.profile.UserInfo(
        name = "Bob",
        lastName = "King",
  ...
  @Test
  fun refresh_fallsBackToAuthDisplayName_whenProfileMissing() = runTest(mainRule.scheduler) {
  ...
  @Test
  fun refresh_withNoAuthenticatedUser_setsNameUserAndEmptyEmail() = runTest(mainRule.scheduler) {
  ...
  fun deleteAccount_successOrRecentLoginError_profileRemoved_flagResets_andCallbackFires()
    ...
    vm.deleteAccount { ok, msg -> cbOk = ok; cbMsg = msg }
  ...
  @Test
  fun deleteAccount_whenNoUser_setsErrorAndKeepsNotDeleting() = runTest(mainRule.scheduler) {
    FirebaseEmulator.auth.signOut()
    val vm = vm()
    var ok: Boolean? = null
    var msg: String? = null

    vm.deleteAccount { success, m -> ok = success; msg = m }
  ...
  @Test
  fun clearError_setsErrorMsgToNull() = runTest(mainRule.scheduler) {
    FirebaseEmulator.auth.signOut()
    val vm = vm()
  ...
  @Test
  fun onItemClick_isNoOp_stateUnchanged() = runTest(mainRule.scheduler) {
    switchToUser(FakeUser.FakeUser2)
    val vm = vm()
    vm.refresh()
